  ...
  ---------------------------------------------------------------------------*/
    ...
    while (members_remaining) {
        ...
        while (members_remaining && (j < DIR_BLKSIZ)) {
            ...
            if ((error = do_string(crec.filename_length,ZFILENAME)) != PK_COOL) {
    ...
    /*-----------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
        ...
        for (i = 0; i < j; ++i) {
            ...
            if ((error = do_string(lrec.filename_length,ZFILENAME)) != PK_COOL) {
                if (error > error_in_archive)
                    error_in_archive = error;
                if (error > PK_WARN) {
...
reprompt:
        ...
        cur_zipfile_bufstart = m_pFile->seekg((LONGINT)cd_bufstart, std::ios::beg ).tellg();
